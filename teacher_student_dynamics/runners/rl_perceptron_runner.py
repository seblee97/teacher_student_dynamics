...
class RLPerceptronRunner(base_network_runner.BaseNetworkRunner):
    """Implementation of RL Perceptron.

    Used in Patel et. al 23.
    """
    ...
    def __init__(self, config: experiments.config.Config, unique_id: str = "") -> None:
        ...
        self._episode_condition = config.condition
        self._reward = config.reward
        self._penalty = config.penalty

        super().__init__(config, unique_id)
        self._logger.info("Setting up rl perceptron runner...")
    ...
    def _setup_network_configuration(self):
        with torch.no_grad():
            ...
            student_layer = self._student.heads[0].weight.data
            student_teacher_overlaps = [
                student_layer.mm(teacher.heads[0].weight.data.t()).cpu().numpy()
                / self._input_dimension
                for teacher in self._teachers.networks
            ]

            return network_configurations.PerceptronNetworkConfiguration(
    ...
    def _training_step(self, teacher_index: int, replaying: Optional[bool] = None):
        ...
        if self._episode_condition == constants.ALL_CORRECT:
            true_condition = all(
                torch.sign(student_output) == torch.sign(teacher_output)
            )
            episode_return = self._reward * true_condition - self._penalty * (
                1 - true_condition
            )
        else:
            raise ValueError(
                f"Episode condition {self._episode_condition} not recognised."
            )

        # training iteration
        self._optimiser.zero_grad()
        loss = -torch.mean(student_output * teacher_output * episode_return)
    ...
    def _compute_generalisation_errors(self) -> List[float]:
        ...
        with torch.no_grad():
            ...
            for i, (student_output, teacher_output) in enumerate(
                ...
                generalisation_errors[f"{constants.GENERALISATION_ERROR}_{i}"] = error
